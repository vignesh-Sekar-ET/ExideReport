...
@Component({
    selector: 'bh-reportupdate',
    templateUrl: './reportupdate.template.html'
})

export class reportupdateComponent extends NBaseComponent implements OnInit {
    mm: ModelMethods;
...
// })
        ...
        let updateid=this.reportserviceupdate.updatename[0].id;
         if(this.reportserviceupdate.updatename[0].Active==1){
         this.reportCreateForm.patchValue({selectStatus:'inactive'})
          }
            else if(this.reportserviceupdate.updatename[0].Active==0){
